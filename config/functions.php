...
function fetch_formations_for_category($category_id, $limit = ''){
    return fetch_all("SELECT * FROM formations, categories_formations WHERE id_category = '$category_id' AND id_formation = formation_id AND deleted = 0 order by date_creation desc $limit");
}

function get_undercat_formation($category_id, $limit = '') {
    $formations = array();

    $cc = fetch_all("SELECT * FROM categories WHERE category_parent = '$category_id' ORDER BY category_name $limit");
    ...
    if (!empty($cc)) {
        foreach ($cc as $c) {
            if (category_has_child($c['category_id'])) {
                ...
                $formationsForCategory = fetch_formations_for_category($c['category_id'], $limit); // Replace with your actual function
    ...
    return array_slice($formations, 0, 8);
}


function start_cache(){
    if(CACHE && PATH != 'contact' && PATH != 'inscription'){

        $cache_filename = CACHE_DIR.str_replace('/','_', PATH).'.html';
...
function end_cache(){
    if(CACHE && PATH != 'contact' && PATH != 'inscription'){
        ...
        $cache_filecontent = ob_get_contents();
    
        file_put_contents($cache_filename, $cache_filecontent);
    
        ob_end_clean();
    
        echo $cache_filecontent;
